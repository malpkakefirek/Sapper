...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_page();
  $$unsubscribe_time();
  return `${$$result.head += `<!-- HEAD_svelte-1plg2z2_START -->${$$result.title = `<title>Friends</title>`, ""}<meta name="description" content="About this app"><!-- HEAD_svelte-1plg2z2_END -->`, ""}  <div class="text-column svelte-1hzn8rv">The mouse position is ${escape(mouse.x)} x ${escape(mouse.y)} <h1>You are in <bold>Friends${escape($page.url.hash)}</bold> page!</h1> <p data-svelte-h="svelte-82huk9">mhm game gaming game game game game game game game game game here</p> <pre>time: ${escape(formatter.format($time))} nice :)</pre> <p data-svelte-h="svelte-1foh4ym">Need change. static to dynamic.</p> <pre data-svelte-h="svelte-8v4j80">DONE</pre> </div>`;
});
export {
  Page as default
};
